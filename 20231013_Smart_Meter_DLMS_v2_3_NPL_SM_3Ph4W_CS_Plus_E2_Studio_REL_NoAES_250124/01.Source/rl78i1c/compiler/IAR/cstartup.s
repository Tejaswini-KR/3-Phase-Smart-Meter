...
;------------------------------------------------------------------------------
        ...
        SECTION    .startup:CODE:ROOT(0)

        PUBLIC  `@cstart`             ; Renesas debugger specific
        PUBLIC  __iar_userprogram_start
...
PMC     DEFINE  0xFFFFE
        ...
        REQUIRE __cstart_tls_init
          ...
          CFI ?RET Undefined
          CFI Function __iar_userprogram_start
          CALL_GRAPH_ROOT __iar_userprogram_start, "bl_main"
;------------------------------------------------------------------------------
;       Perform the run-time initialization.
;------------------------------------------------------------------------------

__iar_userprogram_start:
`@cstart`:
        CODE
        DI
