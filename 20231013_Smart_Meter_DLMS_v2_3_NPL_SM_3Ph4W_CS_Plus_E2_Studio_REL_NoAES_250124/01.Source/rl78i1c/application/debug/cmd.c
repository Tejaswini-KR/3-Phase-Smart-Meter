...
static void s_format_print(const void *format, va_list arg)
{
    #define CMD_PRINTF_MAX_STR_LENGTH       300
...
* Return Value : None
***********************************************************************************************************************/
void CMD_Printf(const void *format, ...)
{
    #ifdef __IAR__
    va_list     arg = { NULL };
    #else
...
{
    uint8_t color_str_identifier[3];
    uint8_t color_command[6];
    #ifdef __IAR__
    va_list     arg = { NULL };
    #else
    va_list     arg = NULL;
    ...
    /* Format color code to string */
    sprintf((char *)color_str_identifier, "%2u", color);

    /* Prepare set color buffer */
    color_command[0] = 0;
    s_stradd((uint8_t *)"\x1b[", color_command);
    s_stradd(color_str_identifier, color_command);
    s_stradd((uint8_t *)"m", color_command);
    
    /* Send color setting command */
    if (color != PRINT_COLOR_VT100_DEFAULT)
    {
        s_sendstring(color_command, 1);
    }

    /* Send string information */
    s_format_print(format, arg);
    
    /* Send color reset command */
    if (color != PRINT_COLOR_VT100_DEFAULT)
    {
        s_sendstring((uint8_t *)"\x1b[0m", 1);
    }
